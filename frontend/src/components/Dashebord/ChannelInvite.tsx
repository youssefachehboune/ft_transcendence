...
export default function FriendRequest(props : Props) {
    ...
    function handleAccept() {
        const data : {name: string, password: string | undefined | null} = {name: props.name, password: undefined};
        fetch(`http://localhost:3000/channel/user/accept`, {
            credentials: "include",
            method: 'PUT',
            headers: {'Content-Type': 'application/json' },
            body: JSON.stringify(data),
          })
        socket.emit('add_channel', props.name);
        setshowRequestFriend(false);
        props.setmychanel((prevChannels: any) => [...prevChannels, props.chanel]);
    }
        ...
        >
            ...
            <div className="w-[73%] h-[100%]">
                <div className="w-[100%] h-[50%]  flex flex-col justify-end items-start">
                    ...
                    <h1 className="text-[7px] text-[#969696]">  </h1>
                </div>
